...
def index_post(request):
	...
	if request.method=='POST':
		try:
			...
			selected_choice=main.objects.get(yourlocation=num)
			context={'data':selected_choice}
		except main.DoesNotExist:
			raise Http404("login DoesNotExist")
...
def govt_page(request):	
	...
	if request.method == 'POST':
		...
		if login.objects.filter(username=user).exists() and login.objects.filter(password=pas).exists():
			...
			context={'error':"enter username or password is wrong"}
